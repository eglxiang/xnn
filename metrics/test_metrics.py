...
def test_binarizedBER():
    ...
    assert np.allclose(bbe(binpred,td),0.416666666667)
...
def test_cce_and_KL():
    ...
    assert np.allclose(cce(probpred,td)[0:4], [1.33217907, 0.50040239, -0., 50.])
    assert np.isnan(cce(probpred,td)[-1])
    #print "bettercce",cce(betterprobpred,td)
    assert np.allclose(cce(betterprobpred,td)[0:4], [0.59191859, 0.50040239, -0., 0.69314718])
    ...
    assert np.allclose(cceMean(probpred,td),12.9581)
    kl = xnn.metrics.Metric('kl','probtarg',aggregation_type='none')
    #print "kl",kl(probpred,td)
    assert np.allclose(kl(probpred,td),[  0.83177662, 0., 0., 50., 0.])
...
def test_optimized_threshold():
    ...
    assert np.allclose(obeMean(binpred,td)[0],0.33333333333333331)
